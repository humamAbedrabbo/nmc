...
namespace NMCUI.Areas.UI1.Pages.Doctors
{
    public class EditDoctorModel : PageModel
    {
        ...
        public EditDoctorModel(IDoctorRepository doctorRepository, IWebHostEnvironment env, UserManager<AppUser> userManager)
        {
        ...
        {
            Doctor = await doctorRepository.GetDoctor(id);
        ...
        public async Task<IActionResult> OnPost()
        {
            if (ModelState.IsValid)
            {
                ...
                // Upload photo
                string photo = Doctor.PhotoPath ?? "user.jpg";
                if (Photo != null && Photo?.Length > 0)
                {
                    string oldPhoto = Doctor.PhotoPath;
                ...
                // Update user
                AppUser user = null;
                if (!string.IsNullOrEmpty(Doctor.Username) && string.IsNullOrEmpty(Username) )
                {
